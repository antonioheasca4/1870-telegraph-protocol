...
│     └─> Dynamic structure: Adapts to JSON fields automatically │
│     └─> Smart grouping: sender_*, receiver_*, other fields     │
│     └─> Type-aware: Lists, dicts, strings rendered properly    │
...
│     └─> Dynamic PDF: Auto-generated from ANY JSON fields       │
...
- **Dynamic PDF generation**: PDF structure adapts automatically to JSON fields
- **Smart field grouping**: Sender/receiver fields auto-detected by prefix
- **Type-aware rendering**: Lists, dictionaries, and simple values rendered appropriately
...
- **Dynamic PDF Structure**: Automatically adapts to any JSON schema
- **Data Flow**: Web form → PDF → Hash → TCP → Reconstruct → Validate

### Dynamic PDF Generation

The system uses **intelligent field detection** to generate PDFs from ANY JSON structure:

**How It Works:**
1. **Field Grouping**: Automatically detects sender/receiver fields by prefix (`sender_*`, `receiver_*`)
2. **Type Detection**: Renders different data types appropriately:
   - **Simple values** (strings, numbers): Displayed in tables
   - **Lists** (line_items, cargo_manifest): Formatted as numbered/bulleted items
   - **Dictionaries** (bank_details): Rendered as subsections with key-value pairs
3. **Smart Sectioning**: Groups related fields into logical sections
4. **No Hardcoded Schema**: PDF structure adapts to whatever fields exist in JSON

**Example JSON → PDF Mapping:**

```json
{
  "sender_name": "Company A",
  "sender_email": "contact@companya.com",
  "receiver_address": "123 Main St",
  "line_items": [
    {"service": "Web Design", "rate": 85.00}
  ],
  "bank_details": {
    "bank_name": "ANZ Bank",
    "account": "12341234"
...
- **Sender Information**: Name, Email (auto-detected from `sender_*`)
- **Receiver Information**: Address (auto-detected from `receiver_*`)
- **Line Items**: Service list with formatted details
- **Bank Details**: Subsection with bank name and account
- **Transaction Details**: Any other simple fields

**Benefits:**
- No template maintenance required
- Works with ANY banking document structure
- Automatically includes new fields when added to JSON
